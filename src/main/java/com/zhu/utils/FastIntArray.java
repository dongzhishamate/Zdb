...
public class FastIntArray implements Iterable<Integer> {
  ...
  public int findEqualOrGreater(int value, int start, int end) {
    if (value > elementData[end - 1]) {
      return -1;
    }
    if (value <= elementData[start]) {
      return start;
    }
    lastPos = bs(start, end, value);
    return lastPos;
  }
